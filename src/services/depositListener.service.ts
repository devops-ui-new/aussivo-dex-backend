...
export class DepositListenerService {
  ...
  constructor() {
    ...
    this.provider.on("error", (err) => {
      logger.error(
        `[DepositListener] RPC provider error: ${(err as Error)?.message || err}`,
      );
    });
  }
  ...
  async start() {
    if (!VAULT_CONTRACT_ADDRESS) {
      logger.warn(
        "[DepositListener] No VAULT_CONTRACT_ADDRESS configured, skipping listener",
      );
      return;
    }
    if (!ethers.isAddress(VAULT_CONTRACT_ADDRESS)) {
      logger.warn(
        "[DepositListener] Invalid VAULT_CONTRACT_ADDRESS configured, skipping listener",
      );
      return;
    }
    ...
    this.isRunning = true;
    logger.info(
      `[DepositListener] Starting... Watching ${VAULT_CONTRACT_ADDRESS}`,
    );
    ...
    try {
      ...
      const handleEvent = async (...listenerArgs: any[]) => {
        try {
          ...
          if (!value || !requestId || requestId === ethers.ZeroHash) {
            logger.info(
              `[DepositListener] Legacy/non-request deposit event tx:${txHash}; requestId=${requestIdRaw || "n/a"} skipped for deterministic routing`,
            );
            return;
          }
          ...
          if (existing) {
            logger.info(
              `[DepositListener] Tx ${txHash} already processed, skipping`,
            );
            return;
          }
          ...
          if (!pending) {
            logger.warn(
              `[DepositListener] No pending intent found for requestId ${requestId} (tx ${txHash})`,
            );
          ...
          if (!user) {
            logger.warn(
              `[DepositListener] Pending intent ${pending._id} has no user ${pending.userId}`,
            );
            return;
          }
          ...
          if (!symbol) {
            logger.warn(
              `[DepositListener] Unsupported token ${tokenAddress} in deposit tx ${txHash}`,
            );
            return;
          }
          if (pending.asset !== symbol) {
            logger.warn(
              `[DepositListener] Asset mismatch for requestId ${requestId}: pending=${pending.asset}, onchain=${symbol}`,
            );
            return;
          }
          ...
          if (pending.expectedAmountBaseUnits !== amountBaseUnits) {
            logger.warn(
              `[DepositListener] Amount mismatch for requestId ${requestId}: pending=${pending.expectedAmountBaseUnits}, onchain(received)=${amountBaseUnits}. Crediting received amount.`,
            );
          }
          ...
          const known = (user.walletAddresses || []).map((w: string) =>
            (w || "").toLowerCase(),
          );
          if (!known.includes(fromLower)) {
            ...
            if (!conflict) {
              ...
              await user.save();
              logger.info(
                `[DepositListener] Auto-linked wallet ${fromLower} to user ${user.email}`,
              );
            }
          }
          ...
          if (!vault) {
            logger.warn(
              `[DepositListener] Vault not found for pending intent ${pending._id}`,
            );
            return;
          }
          ...
          logger.info(
            `[DepositListener] Matched requestId ${requestId} (intent ${pending._id}) → vault ${vault.name}`,
          );
          ...
          logger.info(
            `[DepositListener] ✅ Auto-confirmed requestId ${requestId}: ${amount} ${symbol} for ${user.email} in ${vault.name}`,
          );
        } catch (err: any) {
          logger.error(
            `[DepositListener] Error processing transfer: ${err.message}`,
          );
        }
      };

      // Poll with queryFilter (eth_getLogs) instead of eth_newFilter/eth_getFilterChanges.
      // Public RPCs expire server-side filters ("filter not found") — getLogs is stateless.
      ...
      const poll = async () => {
        try {
          ...
          logger.warn(
            `[DepositListener] poll error (non-fund-critical): ${e?.shortMessage || e?.message || e}`,
          );
        }
      };
      await poll();
      ...
      logger.info(
        "[DepositListener] Watching AussivoVault Deposited events (getLogs polling)",
      );
    } catch (err: any) {
