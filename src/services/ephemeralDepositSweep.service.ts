...
/** BSC gas-funder balance + whether it can still fund sweeps. For the admin monitor. */
export async function getGasFunderStatus(): Promise<{ address: string; bnb: string; ok: boolean } | null> {
  if (!GAS_FUNDER_PRIVATE_KEY) return null;
  try {
    const wallet = new ethers.Wallet(GAS_FUNDER_PRIVATE_KEY);
    const bal = await getProvider().getBalance(wallet.address);
    return { address: wallet.address, bnb: ethers.formatEther(bal), ok: bal > GAS_TOPUP_WEI * 10n };
  } catch { return null; }
}
...
/** Admin action: immediately re-run the sweep for one pending deposit (BSC). */
export async function forceSweepPending(id: string): Promise<void> {
  const doc = await PendingDepositModel.findById(id).lean();
  if (doc) await sweepOne(doc);
}
